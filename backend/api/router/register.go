...
var (
	globalDB *gorm.DB // 全局数据库实例，用于K12管理路由
)

// SetDB 设置全局数据库实例（由application.Init调用）
func SetDB(db *gorm.DB) {
	globalDB = db
	logs.Infof("Router: Database instance set for K12 Admin API")
}
...
func GeneratedRegister(r *server.Hertz) {
	...
	// 注册 K12 管理API路由
	if globalDB != nil {
		registerK12AdminRoutes(r, globalDB)
	} else {
		logs.Warnf("K12 Admin routes not registered: database instance is nil")
	}
	// 注意：需要在main.go中用实际的db实例调用
	// registerK12AdminRoutes(r, db)
...
// registerK12AdminRoutes 注册 K12 管理API路由
func registerK12AdminRoutes(r *server.Hertz, db *gorm.DB) {
	// 创建K12管理handler
	k12AdminHandler, err := cozeHandler.NewK12AdminHandler(db)
	if err != nil {
		logs.Errorf("Failed to create K12 admin handler: %v", err)
		return
	}

	adminAPI := r.Group("/api/adapter/k12/admin")
	{
		// 1. 生成知识图谱
		adminAPI.POST("/knowledge-graph/generate", k12AdminHandler.GenerateKnowledgeGraph)

		// 2. 查询知识点
		adminAPI.GET("/knowledge-points", k12AdminHandler.QueryKnowledgePoints)

		// 3. 刷新缓存
		adminAPI.POST("/cache/refresh", k12AdminHandler.RefreshCache)

		// 4. 清空缓存
		adminAPI.DELETE("/cache/clear", k12AdminHandler.ClearCache)

		// 5. 缓存统计
		adminAPI.GET("/cache/stats", k12AdminHandler.GetCacheStats)

		// 6. 生成日志
		adminAPI.GET("/generation-logs", k12AdminHandler.GetGenerationLogs)

		// 辅助端点
		adminAPI.GET("/subjects", k12AdminHandler.GetSubjects)
		adminAPI.GET("/system-info", k12AdminHandler.GetSystemInfo)
	}

	logs.Infof("K12 Admin API routes registered (8 endpoints)")
}
